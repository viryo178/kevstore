...
<main id="main" class="main">
  ...
  <section class="section dashboard">
    ...
    <div class="row kelola-layout-row">
      <?php
        ...
        <a href="<?= base_url('admin/deactived') ?>" class="stat-card-link">
          ...
          <div class="card info-card customers-card">
            ...
            <div class="card-body">
              ...
              <h5 class="card-title">
                Akun Bermasalah <span>| Total</span>
              </h5>
      ...
      <div class="col-12">
        ...
        <div class="card recent-sales overflow-auto">
          ...
          <div class="card-body">
            ...
            <form class="kelola-date-filter" method="get" action="<?= base_url('admin/kelola_akun') ?>">
              <div>
                <label class="form-label" for="kelola_product">Jenis akun</label>
                <select class="form-control" id="kelola_product" name="product" onchange="this.form.submit()">
                  <option value="">Semua jenis akun</option>
                  <?php foreach (['LEONARDO', 'SPOTIFY', 'GEMINI', 'ZOOM', 'ADOBE'] as $filter_product): ?>
                    <option value="<?= $filter_product ?>" <?= ($selected_product ?? '') === $filter_product ? 'selected' : '' ?>><?= ucfirst(strtolower($filter_product)) ?></option>
                  <?php endforeach; ?>
                </select>
              </div>
              ...
              <a href="<?= base_url('admin/kelola_akun') ?>" class="btn btn-sm kelola-filter-btn kelola-filter-muted">
                  ...
                  <?php foreach ($akun as $a): ?>
                    ...
                    <tr id="akun-row-<?= $a->id_akun ?>"
                      ...
                      <td class="d-flex gap-2">
                        ...
                        <?php else: ?>
                          <a
                            href="<?= base_url('admin/edit_akun/' . $a->id_akun) . '?return_to=' . rawurlencode('admin/kelola_akun' . (!empty($_SERVER['QUERY_STRING']) ? '?' . $_SERVER['QUERY_STRING'] : '')) ?>"
                            class="btn-edit">
...
<div class="modal fade" id="bulkEditModal" tabindex="-1">
  <div class="modal-dialog modal-xl modal-dialog-centered">
    <div class="modal-content">
      <form id="formBulkEditAkun" action="<?= base_url('admin/bulk_edit_akun') ?>" method="POST">
        <div class="modal-header">
...
<script>
  ...
  const editAkunActionBase = '<?= base_url('admin/edit_akun/') ?>';
  const bulkDeleteAction = '<?= base_url('admin/bulk_hapus_akun') ?>';
  ...
  function fillEditAkunModal(id) {
    const account = akunBulkData.find(function(item) {
      return String(item.id_akun) === String(id);
    });
    const modalEl = document.getElementById('editModal');
    const form = document.getElementById('formEditAkun');

    if (!account || !modalEl || !form) {
      showCrudMessage('Data akun tidak ditemukan');
      return;
    }
