...
<style>
/* Warna identitas tiap kartu produk dibuat lebih jelas. */
.dashboard .metric-product .metric-card{position:relative;overflow:hidden;background:#0b1f3a!important}
.dashboard .metric-product .metric-card::before{content:"";position:absolute;inset:0;pointer-events:none;opacity:1}
.dashboard .metric-product .card-body{position:relative;z-index:1}
.dashboard .metric-product .card-title,.dashboard .metric-product .metric-copy h6{color:#fff!important}

.dashboard .product-card-spotify{border-color:#16a34a!important;box-shadow:inset 0 0 28px rgba(34,197,94,.06)}
.dashboard .product-card-spotify::before{background:linear-gradient(135deg,rgba(34,197,94,.13),transparent 58%)}
.dashboard .product-card-spotify .card-icon{background:#123f35!important;color:#4ade80!important;border:1px solid rgba(74,222,128,.35)}

.dashboard .product-card-leonardo{border-color:#ef4444!important;box-shadow:inset 0 0 28px rgba(239,68,68,.06)}
.dashboard .product-card-leonardo::before{background:linear-gradient(135deg,rgba(239,68,68,.13),transparent 58%)}
.dashboard .product-card-leonardo .card-icon{background:#48233a!important;color:#fb7185!important;border:1px solid rgba(251,113,133,.35)}

.dashboard .product-card-gemini{border-color:#eab308!important;box-shadow:inset 0 0 28px rgba(234,179,8,.06)}
.dashboard .product-card-gemini::before{background:linear-gradient(135deg,rgba(234,179,8,.13),transparent 58%)}
.dashboard .product-card-gemini .card-icon{background:#403a20!important;color:#fde047!important;border:1px solid rgba(253,224,71,.35)}

.dashboard .product-card-zoom{border-color:#3b82f6!important;box-shadow:inset 0 0 28px rgba(59,130,246,.07)}
.dashboard .product-card-zoom::before{background:linear-gradient(135deg,rgba(59,130,246,.14),transparent 58%)}
.dashboard .product-card-zoom .card-icon{background:#173b69!important;color:#60a5fa!important;border:1px solid rgba(96,165,250,.4)}

.dashboard .product-card-adobe{border-color:#a855f7!important;box-shadow:inset 0 0 28px rgba(168,85,247,.07)}
.dashboard .product-card-adobe::before{background:linear-gradient(135deg,rgba(168,85,247,.14),transparent 58%)}
.dashboard .product-card-adobe .card-icon{background:#38245d!important;color:#d8b4fe!important;border:1px solid rgba(216,180,254,.38)}
      ...
      <div class="dashboard-notif-list">
        <?php
          ...
          if (!empty($almost_expired)) {
              $dashboard_notification_groups[] = ['id'=>'expired-today','title'=>'Expired Hari Ini','description'=>count($almost_expired).' akun jatuh tempo hari ini','icon'=>'bi-bell-fill','severity'=>'notif-warning','accounts'=>$almost_expired,'meta'=>'expired_password'];
          }
          $problem_status_config = ['verif'=>['Verif','bi-shield-check'],'deactived'=>['Akun Bermasalah','bi-exclamation-octagon-fill'],'tidak_preimum'=>['Tidak Premium','bi-star-fill'],'ban'=>['Ban','bi-slash-circle-fill'],'lainnya'=>['Lainnya','bi-info-circle-fill']];
          ...
          foreach ($problem_status_config as $problem_status => $config) {
              ...
              $dashboard_notification_groups[] = ['id'=>'status-'.$problem_status,'title'=>$config[0],'description'=>count($problem_status_groups[$problem_status]).' akun perlu dicek','icon'=>$config[1],'severity'=>$problem_status === 'deactived' ? 'notif-danger' : 'notif-warning','accounts'=>$problem_status_groups[$problem_status],'meta'=>'status'];
          }
        ?>
        ...
        <?php if (!empty($dashboard_notification_groups)): ?>
          <?php foreach ($dashboard_notification_groups as $index => $notification): ?>
            ...
            <div class="notif-group <?= htmlspecialchars((string) $notification['severity'], ENT_QUOTES, 'UTF-8') ?>">
                    ...
                    <div class="notif-account-item">
                      <div>
                        ...
                        <div class="notif-account-meta">
                          ...
                          - <?= $notification['meta'] === 'status' ? htmlspecialchars(ucwords(str_replace('_', ' ', (string)($account->status ?? '-'))), ENT_QUOTES, 'UTF-8') : (!empty($account->expired_password) ? date('d M Y', strtotime($account->expired_password)) : '-') ?>
                        </div>
                      </div>
                      <a class="btn btn-sm btn-warning btn-notif-edit" href="<?= base_url('admin/edit_akun/' . (int) $account->id_akun) . '?return_to=' . rawurlencode('admin' . (!empty($dashboard_product) ? '?produk=' . $dashboard_product : '')) ?>">Edit</a>
                    </div>
                  <?php endforeach; ?>
                </div>
              </div>
